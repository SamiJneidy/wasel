...
from .exceptions import BaseAppException, InvalidDocumentType, InvoiceNotFoundException, InvoiceSigningError, IntegrityErrorException, InvoiceUpdateNotAllowed, raise_integrity_error
...
class SaleInvoiceService:
    ...
    async def generate_invoice_number(self, user_id: int, document_type: DocumentType, invoice_type: InvoiceType, invoice_type_code: InvoiceTypeCode) -> str:
        ...
        return seq_number, invoice_helper.format_invoice_number(document_type, invoice_type, invoice_type_code, year, seq_number)
    ...
    async def get_invoice(self, invoice_id: int) -> SaleInvoiceOut:
        ...
        invoice_customer = await self.customer_service.get(db_invoice.customer_id) if db_invoice.customer_id else None
    ...
    async def calculate_amounts(self, data: SaleInvoiceCreate) -> dict:
        ...
        for line in data.invoice_lines:
            item_price_afer_price_discount = round_decimal(
                line.item_price - line.price_discount, 2)
            total = round_decimal(
                item_price_afer_price_discount * line.quantity - line.discount_amount, 2)
            tax_rate = TAX_RATE[line.classified_tax_category]

            if (data.prices_include_tax == True):
                line_extension_amount = round_decimal(
                    (total * 100) / (tax_rate + 100), 2)
                tax_amount = round_decimal(total - line_extension_amount, 2)
            else:
                line_extension_amount = total
                tax_amount = round_decimal(
                    line_extension_amount * tax_rate / 100, 2)

            rounding_amount = round_decimal(
                line_extension_amount + tax_amount, 2)
        ...
        # Add invoice totals
        invoice_line_extension_amount = round_decimal(
            sum(line["line_extension_amount"] for line in invoice_lines), 2)
        tax_categories = set(line["classified_tax_category"]
                             for line in invoice_lines)
        if (len(tax_categories) == 1):
            tax_rate = Decimal(settings.STANDARD_TAX_RATE) if tax_categories.pop(
            ) == TaxCategory.S else Decimal("0")
            invoice_taxable_amount = round_decimal(
                invoice_line_extension_amount - data.discount_amount, 2)
            invoice_tax_amount = round_decimal(
                invoice_taxable_amount * tax_rate / 100, 2)
        else:
            invoice_taxable_amount = invoice_line_extension_amount
            invoice_tax_amount = round_decimal(
                sum(line["tax_amount"] for line in invoice_lines), 2)
        invoice_tax_inclusive_amount = round_decimal(
            invoice_taxable_amount + invoice_tax_amount, 2)
    ...
    async def add_additional_fields_to_invoice_dict(self, invoice_dict: dict) -> None:
    ...
    async def sign_and_send_to_zatca(self, invoice_id: int) -> None:
        ...
        if invoice_header.invoice_type == InvoiceType.SIMPLIFIED:
            ...
            base64_invoice = zatca_result.zatca_response.get(
                "clearedInvoice", None)
    ...
    async def create_invoice(self, data: SaleInvoiceCreate) -> SaleInvoiceOut:
        try:
            ...
            await self.add_additional_fields_to_invoice_dict(invoice_dict)
            invoice_lines = invoice_dict.pop("invoice_lines")
            invoice_dict.update({"is_locked": True})
    ...
    async def create_quotation(self, data: SaleInvoiceCreate) -> SaleInvoiceOut:
        try:
            # Calculate invoice amounts
            invoice_dict = await self.calculate_amounts(data)
            invoice_lines = invoice_dict.pop("invoice_lines")
    ...
    async def update_quotation(self, quotation_id: int, data: SaleInvoiceUpdate) -> SaleInvoiceOut:
        try:
            invoice_header = await self.get_invoice_header(quotation_id)
            if not invoice_header:
                raise InvoiceNotFoundException()
            ...
            invoice_dict = await self.calculate_amounts(data)
            invoice_lines = invoice_dict.pop("invoice_lines")
            await self.sale_invoice_repository.update_invoice(quotation_id, invoice_dict)
            await self.sale_invoice_repository.delete_invoice_lines(quotation_id)
            await self.create_invoice_lines(quotation_id, invoice_lines)
            self.db.commit()
            return await self.get_invoice(quotation_id)
        except IntegrityError as e:
            self.db.rollback()
            raise_integrity_error(e)
        except Exception as e:
    ...
    async def convert_quotation(self, quotation_id: int) -> SaleInvoiceOut:
        try:
            invoice_header = await self.get_invoice_header(quotation_id)
            if not invoice_header:
                raise InvoiceNotFoundException()
            ...
            db_invoice = await self.sale_invoice_repository.get_invoice(self.user.id, quotation_id)
            invoice_dict = invoice_header.model_dump()
            invoice_lines = await self.get_invoice_lines(quotation_id)
            invoice_lines_dict = []
            for line in invoice_lines:
                line_dict = line.model_dump()
                line_dict.update({"item_id": line.item.id})
                invoice_lines_dict.append(line_dict)
            invoice_dict.update({"document_type": DocumentType.INVOICE, "customer_id": db_invoice.customer_id, "invoice_lines": invoice_lines_dict})
    ...
    async def delete_quotation(self, quotation_id: int) -> None:
        try:
            db_invoice = await self.get_invoice_header(quotation_id)
            if not db_invoice:
                raise InvoiceNotFoundException()
            if db_invoice.is_locked == True or db_invoice.document_type != DocumentType.QUOTATION:
                raise InvoiceUpdateNotAllowed()
            await self.sale_invoice_repository.delete_invoice(quotation_id)
            return None
        except IntegrityError as e:
            self.db.rollback()
            raise_integrity_error(e)
