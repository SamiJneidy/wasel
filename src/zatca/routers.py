...
from .dependencies import Annotated, Depends, ZatcaService, get_zatca_service
from .schemas import ZatcaBranchMetadataCreate, ZatcaBranchMetadataOut
...
@router.get(
    path="/{id}",
    response_model=SingleObjectResponse[ZatcaBranchMetadataOut],
...
) -> SingleObjectResponse[ZatcaBranchMetadataOut]:
    data = await zatca_service.get_branch_metadata(current_user, id)
...
async def create_metadata(
    branch_id: int,
    body: ZatcaBranchMetadataCreate,
    zatca_service: Annotated[ZatcaService, Depends(get_zatca_service)],
    current_user: Annotated[UserInDB, Depends(get_current_user)],
) -> SingleObjectResponse[ZatcaBranchMetadataOut]:
