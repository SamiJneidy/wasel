...
from .models import ZatcaBranchMetadata, ZatcaCSID, ZatcaSaleInvoiceLineMetadata, ZatcaSaleInvoiceMetadata
...
class ZatcaRepository:
    ...
    async def get_csid(self, id: int, stage: ZatcaStage) -> ZatcaCSID | None:
        stmt = (
            select(ZatcaCSID)
    ...
    async def get_csid_by_branch(self, organization_id: int, branch_id: int, stage: ZatcaStage) -> ZatcaCSID | None:
        stmt = (
            select(ZatcaCSID)
    ...
    async def get_pih(self, organization_id: int, branch_id: int, stage: ZatcaStage) -> Optional[str]:
    ...
    async def get_icv(self, organization_id: int, branch_id: int, stage: ZatcaStage) -> Optional[int]:
    ...
    async def create_invoice_metadata(self, invoice_id: int, data: dict) -> ZatcaSaleInvoiceMetadata:
        stmt = insert(ZatcaSaleInvoiceMetadata).values(invoice_id=invoice_id, **data).returning(ZatcaSaleInvoiceMetadata)
        result = await self.db.execute(stmt)
        await self.db.flush()
        return result.scalars().first()
    ...
    async def get_invoice_metadata(self, invoice_id: int) -> Optional[ZatcaSaleInvoiceMetadata]:
        stmt = select(ZatcaSaleInvoiceMetadata).where(ZatcaSaleInvoiceMetadata.invoice_id==invoice_id)
        result = await self.db.execute(stmt)
        return result.scalars().first()
    ...
    async def delete_invoice_metadata(self, invoice_id: int) -> None:
        stmt = delete(ZatcaSaleInvoiceMetadata).where(ZatcaSaleInvoiceMetadata.invoice_id==invoice_id)
        await self.db.execute(stmt)
        await self.db.flush()
        return None
