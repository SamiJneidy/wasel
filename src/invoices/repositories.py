...
from .models import Invoice, CustomerSnapshot, InvoiceLine
...
class InvoiceRepository:
    ...
    async def get_invoice_customer(self, id: int) -> CustomerSnapshot | None:
        return self.db.query(CustomerSnapshot).filter(CustomerSnapshot.id==id).first()
    
    async def get_invoice_customer_by_invoice_id(self, invoice_id: int) -> CustomerSnapshot | None:
        return self.db.query(CustomerSnapshot).filter(CustomerSnapshot.invoice_id==invoice_id).first()
    ...
    async def get_invoices_by_user_id(self, user_id: int, stage: Stage, skip: int = None, limit: int = None, filters: dict = {}) -> tuple[int, list[Invoice]]:
        query = self.db.query(Invoice).filter(Invoice.user_id==user_id, Invoice.stage==stage)
        # simple equality filters
        for k, v in filters.items():
            column = getattr(Invoice, k, None)
            if column is not None:
                query = query.filter(column == v)
        # date range filters
        if filters.get("issue_date_range_from") is not None:
            query = query.filter(Invoice.issue_date >= filters["issue_date_range_from"])
        if filters.get("issue_date_range_to") is not None:
            query = query.filter(Invoice.issue_date <= filters["issue_date_range_to"])
    ...
    async def create_invoice_customer(self, data: dict) -> CustomerSnapshot:
