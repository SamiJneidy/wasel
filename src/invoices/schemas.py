...
class InvoiceCreate(InvoiceHeaderBase):
    customer: Union[int, TaxExcemptionCustomer, None] = None
    ...
    def validate_model(self) -> Self:
        ...
        if self.invoice_type == InvoiceType.STANDARD:
            if not isinstance(self.customer, int):
                raise ValueError("Youe must specify the customer id in the standard invoice")
        
        if self.invoice_type == InvoiceType.SIMPLIFIED:
            if (self.classified_tax_category != TaxCategory.S and 
                self.tax_exemption_reason_code in (TaxExemptionReasonCode.VATEX_SA_EDU, TaxExemptionReasonCode.VATEX_SA_EDU) and 
                not isinstance(self.customer, TaxExcemptionCustomer)
            ):
                raise ValueError("You must add customer info to simplified invoices excepmted from VAT")
        
        if self.classified_tax_category == TaxCategory.S and (self.tax_exemption_reason != None or self.tax_exemption_reason_code != None):
            raise ValueError("You can not specify tax exemption reason or tax exemption reason code if the tax category is not E or Z")
        
        if (self.tax_exemption_reason_code in (TaxExemptionReasonCode.VATEX_SA_EDU, TaxExemptionReasonCode.VATEX_SA_HEA) and
            self.customer.party_identification_scheme != PartyIdentificationScheme.NAT
        ):
            raise ValueError("If the invoice tax exemption reason code is VATEX_SA_EDU or VATEX_SA_HEA, the customer identification scheme must be NAT")

        return self
    
class InvoiceOut(InvoiceHeaderOut):
    id: int = Field(...)
    customer: Union[CustomerSnapshotOut, TaxExcemptionCustomerOut, None] = None
